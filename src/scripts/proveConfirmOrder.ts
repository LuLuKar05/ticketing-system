...
async function seed() {
    await AppDataSource.getRepository(Concert).save({
        id: C1, name: 'Confirm', concertDate: new Date(), description: 'x', imageUrl: 'http://x',
        location: 'T', artist: ['A'], genre: ['rock'], totalTickets: START_QTY, duration: 120,
        ageRestriction: 0, oneTicketPerUser: false,
    } as any);
    await AppDataSource.getRepository(TicketTier).save({ id: T1, name: 'General', price: PRICE, quantity: START_QTY, concert: { id: C1 } } as any);
    await AppDataSource.getRepository(User).save({
        id: U1, name: 'U', email: 'confirm@test.local', password: 'x', phoneNumber: '0',
        address: 'h', dateOfBirth: new Date('1990-01-01'),
    } as any);
}

const qty = async () => (await AppDataSource.getRepository(TicketTier).findOneByOrFail({ id: T1 })).quantity;
const soldCount = async () => AppDataSource.getRepository(Ticket).count({ where: { concert: { id: C1 }, status: 'sold' as any } });
...
async function main() {
    ...
    const { order: o1 } = await reserveSvc.reserveTickets({ userId: U1, concertId: C1, seats: [{ tierId: T1, seatNumber: 'A1' }, { tierId: T1, seatNumber: 'A2' }] });
    ...
    (res1.order.totalAmount === 2 * PRICE) ? ok('totalAmount = ' + res1.order.totalAmount) : bad('totalAmount = ' + res1.order.totalAmount);
    ((await soldCount()) === 2) ? ok('2 SOLD tickets in DB') : bad('sold count = ' + (await soldCount()));
    ((await qty()) === START_QTY - 2) ? ok('tier quantity ' + START_QTY + '->' + (await qty())) : bad('tier qty = ' + (await qty()));
    const confirmedReserves = await AppDataSource.getRepository(Reserve).count({ where: { order: { id: o1.id }, status: 'confirmed' as any } });
    ...
    const { order: o2 } = await reserveSvc.reserveTickets({ userId: U1, concertId: C1, seats: [{ tierId: T1, seatNumber: 'X1' }] });
    await ticketRepo.createSoldTicket({ concertId: C1, seatNumber: 'X1', userId: U1, ticketTierId: T1, pricePaid: PRICE, orderId: o2.id }); // simulate other buyer
    ...
    try {
        ...
        (e instanceof SeatsUnavailableError && e.reason === 'sold') ? ok('SeatsUnavailableError sold [' + e.seatNumbers.join(',') + ']') : bad('wrong error: ' + (e as Error).message);
    }
    ...
    ((await qty()) === qtyBeforeB) ? ok('tier quantity unchanged (rolled back)') : bad('tier qty changed ' + qtyBeforeB + '->' + (await qty()));
    ...
    try {
        ...
        (e instanceof TicketUnavailableError) ? ok('rejected double-confirm') : bad('wrong error: ' + (e as Error).message);
    }

    // D. expired hold
    console.log('\n[D] expired hold');
    const { order: o4 } = await reserveSvc.reserveTickets({ userId: U1, concertId: C1, seats: [{ tierId: T1, seatNumber: 'Y1' }] });
    await AppDataSource.getRepository(Reserve).update({ order: { id: o4.id } }, { expiresAt: new Date(Date.now() - 1000) });
    try {
