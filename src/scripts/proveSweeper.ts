...
async function seed() {
    await AppDataSource.getRepository(Concert).save({
        id: C1, name: 'Sweep', concertDate: new Date(), description: 'x', imageUrl: 'http://x',
        location: 'T', artist: ['A'], genre: ['rock'], totalTickets: 100, duration: 120,
        ageRestriction: 0, oneTicketPerUser: false,
    } as any);
    await AppDataSource.getRepository(TicketTier).save({ id: T1, name: 'General', price: 5000, quantity: 100, concert: { id: C1 } } as any);
    await AppDataSource.getRepository(User).save({
        id: U1, name: 'U', email: 'sweep@test.local', password: 'x', phoneNumber: '0',
...
async function main() {
    ...
    const { order: o1 } = await reserveSvc.reserveTickets({ userId: U1, concertId: C1, seats: [{ tierId: T1, seatNumber: 'S1' }] });
    ...
    const { order: o2 } = await reserveSvc.reserveTickets({ userId: U1, concertId: C1, seats: [{ tierId: T1, seatNumber: 'S2' }] });
    ...
    (r2.status === 'pending') ? ok('fresh reserve untouched (still PENDING)') : bad('fresh reserve status = ' + r2.status);
    const ord2 = await orderRepo.findOneByOrFail({ id: o2.id });
    (ord2.status === 'pending') ? ok('fresh order untouched (still PENDING)') : bad('fresh order status = ' + ord2.status);
