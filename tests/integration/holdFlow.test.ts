...
describe('Hold flow (integration, real in-memory DB)', () => {
    ...
    it('rejects holding a SOLD seat (SeatsUnavailableError sold)', async () => {
        ...
        await expect(reserveSvc.reserveTickets({ userId, concertId, seats: ['A1'] })).rejects.toMatchObject({
            name: 'SeatsUnavailableError',
            reason: 'sold',
            seatNumbers: ['A1'],
        });
    ...
    describe('oneTicketPerUser', () => {
        it('rejects an order of >1 seat', async () => {
            const { concertId, userId } = await seedBasic(ds, { oneTicketPerUser: true });
            await expect(reserveSvc.reserveTickets({ userId, concertId, seats: ['A1', 'A2'] })).rejects.toMatchObject({
